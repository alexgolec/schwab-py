...
def latest_order_main(sys_args):
    ...
    required.add_argument('--api_secret', required=True)

    parser.add_argument('--account_hash', type=int,
            help='Restrict lookups to a specific account ID')

    args = parser.parse_args(args=sys_args)
    client = client_from_token_file(args.token_file, args.api_key, args.api_secret)
    hash = client.get_account_numbers().json()[0]['hashValue']

    if args.account_hash:
        orders = client.get_orders_for_account(args.account_hash).json()
        if 'error' in orders:
            print(('TDA returned error: "{}", This is most often caused by ' +
                   'an invalid account ID').format(orders['error']))
            return -1
    else:
        orders = client.get_orders_for_all_linked_accounts().json() 
        if 'error' in orders:
