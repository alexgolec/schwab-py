...
def __update_token(token_path):
    def update_token(t, *args, **kwargs):  # pylint: disable=unused-argument
...
class TokenMetadata:
    ...
    def from_loaded_token(cls, token, unwrapped_token_write_func):
        ...
        if 'creation_timestamp' not in token:
            raise ValueError('WARNING: The token format has changed since this token '
...
def client_from_login_flow(api_key, app_secret, callback_url, token_path,
    ...
    **Important Note:** This method operates by starting an HTTP server on the
    ...
    if parsed.hostname != '127.0.0.1':
        ...
        raise ValueError(('Disallowed hostname {}. client_from_login_flow only allows '
        ...
        while True:
            ...
            try:
                with warnings.catch_warnings():
                    ...
                    httpx.get(
                        f'https://127.0.0.1:{callback_port}/schwab-py-internal/status', verify=False)
                break
...
def easy_client(api_key, app_secret, callback_url, token_path, asyncio=False,
    ...
    *Reminder:* You should never create the token file yourself or modify it in
    ...
    if os.path.isfile(token_path):
        ...
        if max_token_age > 0 <= c.token_age() >= max_token_age:
